...
async fn fetch_repo_contents(
    ...
    let mut request = client.get(&url).header("User-Agent", "repo-downloader");
    ...
    Box::pin(async move {
        ...
        for content in contents {
            if content.content_type == "dir" {
                ...
                let dir_contents =
                    fetch_repo_contents(client, username, repo, &content.path).await?;
                ...
                if let Some(download_url) = &content.download_url {
                    if is_text_file(&content.path) {
                        ...
                        file_request =
                            file_request.header("Authorization", format!("token {}", github_token));
                        ...
                        markdown.push_str(&format!(
                            "Binary file, content not included. [Download link]({})\n\n",
                            download_url
                        ));
                    }
                } else {
            ...
            if content.content_type == "dir" {
                ...
                if let Some(download_url) = &content.download_url {
                    if is_text_file(&content.path) {
                        ...
                        text.push_str(&format!(
                            "Binary file, content not included. Download URL: {}\n\n",
                            download_url
                        ));
                    }
                } else {
